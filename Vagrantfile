...
#      echo "deb http://apt.kubernetes.io/ kubernetes-xenial main" >> ~/kubernetes.list
      ...
      echo "export KUBERNETES_SERVICE_HOST=192.168.8.10" > /etc/profile.d/kubernetes.sh
      echo "export KUBERNETES_SERVICE_PORT=6443" >> /etc/profile.d/kubernetes.sh
      sudo kubeadm join --skip-preflight-checks --token=54c315.78a320e33baaf27d 192.168.8.10:6443 --discovery-token-unsafe-skip-ca-verification
      sleep 120
  ...
  config.vm.define "k8sworker2" do |k8sworker2|
    k8sworker2.ssh.forward_agent = true
    k8sworker2.vm.synced_folder "#{hostMount}", "#{guestMount}"
    k8sworker2.vm.provision "shell", inline: <<-SHELL
      set -e
      set -x
      sed 's/127\.0\.0\.1.*k8s.*/192\.168\.8\.12 k8sworker2/' -i /etc/hosts
      #echo "192.168.8.10	k8smaster" >> /etc/hosts
      curl -s https://packages.cloud.google.com/apt/doc/apt-key.gpg | sudo apt-key add -
 #     echo "deb http://apt.kubernetes.io/ kubernetes-xenial main" >> ~/kubernetes.list
      echo "deb https://packages.cloud.google.com/apt/ kubernetes-xenial-unstable main" >> ~/kubernetes.list
    ...
    if dockerLogin then
          k8sworker2.vm.provision "shell", env: {"USERNAME" => Username.new, "PASSWORD" => Password.new}, inline: <<-SHELL
          set +x
          docker login -u $USERNAME -p $PASSWORD
        SHELL
    end
